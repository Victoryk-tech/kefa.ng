...
import { truncateString } from ".././shared/index";
...
const RegisteredUsers = () => {
  const { users, getUserTotal, getUsers, error, isLoading } =
  ...
  return (
    <div className="relative">
      ...
      {/* search result label */}
      <div>
        {searchUser.length && searchOutput.length > 1 ? (
          <span className="px-3 flex items-center">
            <TbShoppingBagSearch /> search result for "{searchUser}"...
          </span>
        ) : null}
      </div>
      ...
      <div className=" p-1 items-center text-sm flex justify-around w-full ">
        <p className="flex justify-center items-center h-10">
          <img
            src={profilePicture}
            alt="image"
            className="w-full h-full object-contain top"
          />
        </p>
